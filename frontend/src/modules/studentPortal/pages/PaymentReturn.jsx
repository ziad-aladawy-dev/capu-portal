  ...
  if (!orderId) {
    view = (
      <ReturnState icon={<XCircle size={48} />} cls="fail"
        title={t("payments.return_no_order", { defaultValue: "No order to confirm" })}
        text={t("payments.return_no_order_hint", { defaultValue: "We couldn't find a payment to confirm." })} />
    );
  } else if (order?.status === ORDER_STATUS.Paid) {
    view = (
      <ReturnState icon={<CheckCircle2 size={48} />} cls="ok"
        title={t("payments.return_paid", { defaultValue: "Payment successful" })}
        text={t("payments.return_paid_hint", { defaultValue: "Your fees have been paid. A receipt will be available shortly." })}
        amount={order.totalAmount} />
    );
  } else if (order && (order.status === ORDER_STATUS.Failed || order.status === ORDER_STATUS.Expired)) {
    view = (
      <ReturnState icon={<XCircle size={48} />} cls="fail"
        title={t("payments.return_failed", { defaultValue: "Payment not completed" })}
        text={t("payments.return_failed_hint", { defaultValue: "Your payment didn't go through. Your fees are still due — you can try again." })} />
    );
  } else if (order && (order.status === ORDER_STATUS.Cancelled || order.status === ORDER_STATUS.Refunded)) {
    view = (
      <ReturnState icon={<XCircle size={48} />} cls="fail"
        title={t("payments.return_cancelled", { defaultValue: "Order closed" })}
        text={t("payments.return_cancelled_hint", { defaultValue: "This order is no longer active." })} />
    );
  } else if (timedOut) {
    view = (
      <ReturnState icon={<Clock size={48} />} cls="pending"
        title={t("payments.return_pending", { defaultValue: "Still processing" })}
        text={t("payments.return_pending_hint", { defaultValue: "We haven't received confirmation yet. It may take a few minutes — check your Orders shortly." })} />
    );
  } else {
    view = (
      <ReturnState icon={<Loader2 size={48} className="sp-spin" />} cls="pending"
        title={t("payments.return_confirming", { defaultValue: "Confirming your payment…" })}
        text={t("payments.return_confirming_hint", { defaultValue: "Please wait while we verify with HU Treasury. Don't close this page." })} />
    );
  }
      ...
      <div className="sp-return-wrap">
        ...
        {!isLoading && (
          <Link to="/student/payments" className="sp-return-link">
            {t("payments.back_to_payments", { defaultValue: "Back to Payments" })}
          </Link>
        )}
      </div>
    </div>
