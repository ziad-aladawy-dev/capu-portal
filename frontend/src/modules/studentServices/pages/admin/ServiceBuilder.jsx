...
const ServiceBuilder = () => {
  ...
  const validateStep = (currentStep = step) => { if (currentStep===1 && !formData.name.trim()) { setError("Service Name is required"); return false; } if (currentStep===2 && formData.scopeNodeIds.length===0) { setError("Please select at least one structure node"); return false; } if (currentStep===2 && formData.isPaid && formData.price<=0) { setError("Price must be greater than zero"); return false; } if (currentStep===3 && (!formData.workflow?.steps?.length)) { setError("Please add at least one workflow step"); return false; } return true; };
