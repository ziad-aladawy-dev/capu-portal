...
import { ChevronRight, CheckCircle, Trash2, Settings, Shield, GitBranch } from "lucide-react";
...
const BasicInfo = ({ t, formData, updateField, serviceTypes }) => (
  <><div className="sb-card-header"><div className="sb-card-icon"><Settings size={16} /></div><div><p className="sb-card-title">{t("basic_info")}</p><p className="sb-card-subtitle">{t("basic_info_desc")}</p></div></div>
...
const EligibilityPricing = ({ t, formData, updateField, academicYears, semesters, handleYearChange, removeScopeNode, scopeNodeNames, setShowScopeTree }) => (
  <><div className="sb-card-header"><div className="sb-card-icon"><Shield size={16} /></div><div><p className="sb-card-title">{t("eligibility_pricing")}</p><p className="sb-card-subtitle">{t("eligibility_desc")}</p></div></div>
  <div className="sb-card-body"><div className="sb-form-grid">
    <div className="sb-form-group full"><label>{t("structural_scope")}</label><div className="sb-scope-selector"><button type="button" onClick={() => setShowScopeTree(true)} className="btn-outline">+ {t("select_structure_nodes")}</button>
      {formData.scopeNodeIds.length > 0 && (<div className="sb-scope-chips">{formData.scopeNodeIds.map(nodeId => (<span key={nodeId} className="sb-scope-chip">{scopeNodeNames[nodeId] || nodeId}<button onClick={() => removeScopeNode(nodeId)}><Trash2 size={11} /></button></span>))}</div>)}
...
const WorkflowStep = ({ t, formData, updateField }) => (
  <><div className="sb-card-header"><div className="sb-card-icon"><GitBranch size={16} /></div><div><p className="sb-card-title">{t("workflow_builder")}</p><p className="sb-card-subtitle">{t("workflow_desc")}</p></div></div>
...
const ServiceBuilder = () => {
  ...
  const [academicYears, setAcademicYears] = useState([]); const [semesters, setSemesters] = useState([]); const [showScopeTree, setShowScopeTree] = useState(false);
  // Selected scope nodes as an id → name map so removing one never deletes a
  // namesake (tracking bare name strings did).
  ...
  const loadAcademicYears = async () => { try { setAcademicYears(await getAcademicYears()); } catch (e) { console.error(e); } };
  const loadSemesters = async (yearId) => { if (!yearId) { setSemesters([]); return; } try { setSemesters(await getSemestersByYear(yearId)); } catch (e) { console.error(e); } };

  // `loading` is initialized to !!id, so no synchronous setState is needed here.
  const loadService = async () => {
    try {
      ...
      // Workflow: convert fieldType ints back to the builder's string names and
      // surface isRequired as `required`.
  ...
  const cleanWorkflowForApi = (workflow) => {
    ...
    return {
      ...
      steps: workflow.steps.map(rawStep => {
        ...
        return {
          ...wfStep,
          stepType: typeof wfStep.stepType === "string" ? (WORKFLOW_STEP_TYPE[wfStep.stepType] || WORKFLOW_STEP_TYPE.Form) : wfStep.stepType,
          fields: (wfStep.fields || []).map(rawField => {
            const field = stripId(rawField);
            return {
              ...field,
              fieldType: typeof field.type === "string" ? (STEP_FIELD_TYPE[field.type] || STEP_FIELD_TYPE.Text) : field.fieldType,
  ...
  const handleSave = async () => {
    // Validate every step (incl. the workflow) — not just the visible one.
    for (let s = 1; s <= 3; s++) {
    ...
    try {
      ...
      if (id) await editService(id, payload); else await addService(payload);
      navigate("/admin/student-services/services");
    } catch (e) {
      setError(e.response?.data?.title || e.response?.data?.message || e.message || t("something_went_wrong"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="service-builder-container">
      <div className="sb-header"><div className="sb-header-icon"><Settings size={16} /></div><div className="sb-header-text"><h1>{id ? t("edit_service") : t("create_service")}</h1><p>{id ? t("edit_service_desc") : t("create_service_desc")}</p></div></div>
      {error && <div className="sb-error">{error}</div>}
      <div className="sb-stepper">{STEPS.map((s, idx) => (<div key={s.key} className={`sb-step ${step === idx + 1 ? "active" : step > idx + 1 ? "done" : ""}`} onClick={() => { if (idx + 1 <= step) setStep(idx + 1); }}><div className="sb-step-number">{step > idx + 1 ? <CheckCircle size={13} /> : idx + 1}</div><span>{t(s.key)}</span>{idx < 2 && <ChevronRight size={13} className="sb-step-arrow" />}</div>))}</div>
      <div className="sb-step-content">{step === 1 && <BasicInfo t={t} formData={formData} updateField={updateField} serviceTypes={serviceTypes} />}{step === 2 && <EligibilityPricing t={t} formData={formData} updateField={updateField} academicYears={academicYears} semesters={semesters} handleYearChange={handleYearChange} removeScopeNode={removeScopeNode} scopeNodeNames={scopeNodeNames} setShowScopeTree={setShowScopeTree} />}{step === 3 && <WorkflowStep t={t} formData={formData} updateField={updateField} />}</div>
      <div className="sb-actions"><button className="btn-secondary" onClick={() => setStep(s => Math.max(1, s - 1))} disabled={step === 1}>{t("previous")}</button>{step < 3 ? <button className="btn-primary" onClick={handleNext}>{t("next")} <ChevronRight size={14} /></button> : <button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? t("saving") : t("submit")}</button>}</div>
