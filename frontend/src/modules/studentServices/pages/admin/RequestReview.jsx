...
const RequestReview = () => {
  ...
  return (
    <div className="rr-container"><div className="rr-header"><h1>{t("request")} #{currentRequest.requestNumber}</h1></div>
      ...
      <div className="rr-info-card"><h3>{t("payment_info")}</h3><div><strong>{t("payment_status")}:</strong> <StatusBadge status={currentRequest.paymentStatus} type="payment" /></div>{!isFree && <div><strong>{t("amount")}:</strong> ${price}</div>}</div>
