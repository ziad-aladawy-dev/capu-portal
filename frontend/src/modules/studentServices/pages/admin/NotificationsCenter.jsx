...
const NotificationsCenter = () => {
  ...
  const loadNotifications = async () => { setLoading(true); try { const response = await apiClient.get("/notifications"); setNotifications(response.data.items || []); } catch (err) { console.error("Failed to load notifications", err); } finally { setLoading(false); } };
  const markAsRead = async (id) => { try { await apiClient.put(`/api/notifications/${id}/read`); await loadNotifications(); } catch (err) {} };
  const markAllAsRead = async () => { try { await apiClient.put("/notifications/read", { ids: notifications.filter(n=>!n.isRead).map(n=>n.id) }); await loadNotifications(); } catch (err) {} };
  ...
  return (
    <div className="nc-container"><PageHeader icon={Bell} title={<>{t("notifications")}<span className="nc-badge">{notifications.filter(n=>!n.isRead).length}</span></>} actions={<button className="nc-mark-all" onClick={markAllAsRead}><CheckCheck size={16} /> {t("mark_all_read")}</button>} />
