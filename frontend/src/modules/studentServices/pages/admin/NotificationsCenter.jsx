    ...
    <div className="nc-container">
      <div className="nc-header">
        <div className="nc-title">
          ...
          <span className="nc-badge">{notifications.filter(n => !n.isRead).length}</span>
        </div>
        <button className="nc-mark-all" onClick={markAllAsRead}>
        ...
        {filtered.map(notif => (
          <div key={notif.id} className={`nc-item ${!notif.isRead ? "nc-unread" : ""}`}>
            ...
            <div className="nc-actions-buttons">
              {!notif.isRead && (
                <button className="nc-read-btn" onClick={() => markAsRead(notif.id)}>
                  <CheckCheck size={16} />
                </button>
              )}
              <button className="nc-delete-btn" onClick={() => deleteNotification(notif.id)}>
                <Trash2 size={16} />
              </button>
            </div>
          </div>
