...
public class StudentFeesController : ControllerBase
{
    ...
    private readonly ITreasuryStatsQueryService _stats;

    public StudentFeesController(IStudentFeeQueryService fees, ITreasuryStatsQueryService stats)
    {
        _fees = fees;
        _stats = stats;
    }

    /// <summary>Institution-wide fee/order/revenue aggregates for the finance dashboard.</summary>
    [HttpGet("stats")]
    [HasPermission(PermissionNames.PaymentTransactions.View)]
    public async Task<IActionResult> GetStats(CancellationToken cancellationToken)
    {
        var result = await _stats.GetStatsAsync(cancellationToken);
        return Ok(result);
    }
