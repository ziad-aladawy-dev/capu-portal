using CapitalUniversity.Sync.Finance.Domain;

namespace CapitalUniversity.Sync.Finance.Sources;

/// <summary>
/// Push counterpart to <see cref="IExternalInvoiceSource"/>. See
/// <c>IExternalStudentSink</c> for the full idempotency-contract narrative.
/// <para>
/// <b>Idempotency contract (REQUIRED).</b> Implementations MUST dedup on
/// <paramref name="idempotencyKey"/> — the outbox writer passes the outbox
/// row's stable <c>Id</c>.
/// </para>
/// </summary>
public interface IExternalInvoiceSink
{
    Task PushAsync(ExternalInvoice payload, string idempotencyKey, CancellationToken cancellationToken);
}
