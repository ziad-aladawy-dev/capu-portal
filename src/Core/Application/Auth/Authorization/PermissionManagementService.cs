...
public class PermissionManagementService : IPermissionManagementService
{
    ...
    {
        var domain = query.AllFaculties ? "Global" : (query.FacultyId?.ToString() ?? "Global");
        ...
        var roles = await _dbContext.StaffRoles
            .Where(sr => sr.StaffId == query.UserId && sr.Domain == domain && sr.Year == year && sr.Semester == semester)
            .ToListAsync(cancellationToken);

        var overrides = await _dbContext.StaffPermissions
            .Where(sp => sp.StaffId == query.UserId && sp.Domain == domain && sp.Year == year && sp.Semester == semester)
            .ToListAsync(cancellationToken);
    ...
    public async Task<PermissionAssignmentResponse> CreateAssignmentAsync(CreatePermissionAssignmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var domain = request.StructuralScope.AllFaculties ? "Global" : (request.StructuralScope.FacultyId?.ToString() ?? "Global");
        ...
        var existingRoles = await _dbContext.StaffRoles
            .Where(sr => sr.StaffId == request.UserId && sr.Domain == domain && sr.Year == year && sr.Semester == semester)
            .Select(sr => sr.RoleId)
            .ToListAsync(cancellationToken);
        ...
        foreach (var roleId in request.RoleIds)
        {
            ...
            {
                var roleAssignment = new StaffRoleAssignment(request.UserId, roleId, domain, year, semester)
                {
        ...
        var existingOverrides = await _dbContext.StaffPermissions
            .Where(sp => sp.StaffId == request.UserId && sp.Domain == domain && sp.Year == year && sp.Semester == semester)
            .ToListAsync(cancellationToken);
        ...
        foreach (var perm in request.PermissionOverrides)
        {
            ...
            {
                var spOverride = new StaffPermissionOverride(request.UserId, perm.ServiceId, perm.Resource, perm.Level, perm.Type, domain, year, semester);
    ...
    public async Task<PermissionAssignmentResponse> UpdateAssignmentAsync(UpdatePermissionAssignmentRequest request, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var permToAdd in request.PermissionsToAdd)
        {
            ...
            {
                var spOverride = new StaffPermissionOverride(request.UserId, permToAdd.ServiceId, permToAdd.Resource, permToAdd.Level, permToAdd.Type, domain, year, semester);
